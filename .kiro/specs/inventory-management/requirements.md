...
- **機器コード**: 個別機器を識別するための一意のコード（AL1、NAL1、Monitor1など）
- **AL**: Amazon Loaner PCの略称（機器コードの接頭辞）
- **NAL**: Non-Amazon Loaner PCの略称（機器コードの接頭辞）
- **個別機器管理**: 各機器を個別に追跡・管理する機能
- **機器ステータス**: 個別機器の現在の状態（利用可能、使用中、メンテナンス中、廃棄済み）
- **使用履歴**: 個別機器の過去の利用記録
- **機器割り当て**: 予約確定時に具体的な機器を予約に紐付ける処理
...
**ユーザーストーリー:** ITチーム管理者として、個別の機器を詳細に管理したい。そうすることで、どの機器を誰が使用しているかを正確に把握し、機器の使用履歴を追跡できる。

#### 受入基準

1. WHEN 管理者が個別機器管理画面にアクセスする THEN システムは全機器の一覧を機器コード（AL1、NAL1、Monitor1など）と現在のステータスで表示する
2. WHEN 管理者が新しい機器を登録する THEN システムは機器コード、機器タイプ、サイト、シリアル番号、モデル情報を記録する
3. WHEN 予約が確定される THEN システムは利用可能な機器を自動的に割り当て、機器ステータスを「使用中」に更新する
4. WHEN 機器が返却される THEN システムは機器ステータスを「利用可能」に更新し、使用履歴に終了日時を記録する
5. WHEN 管理者が機器の詳細を確認する THEN システムは機器の基本情報、現在の使用者、使用履歴を表示する
6. WHEN 管理者が使用中機器一覧を確認する THEN システムは機器コード、使用者Alias、開始日、予定返却日、遅延状況を表示する
7. WHEN 管理者が機器をメンテナンス状態に設定する THEN システムは機器ステータスを「メンテナンス中」に更新し、予約割り当て対象から除外する
8. WHEN 管理者が機器の使用履歴を確認する THEN システムは過去の全ての使用記録を時系列で表示する
9. WHEN 予約者が予約詳細を確認する THEN システムは割り当てられた具体的な機器コードを表示する
10. WHEN システムが機器割り当てを行う THEN システムは同一サイト内で利用可能な機器を優先的に選択する

### 要件20

**ユーザーストーリー:** 予約者として、自分に割り当てられた具体的な機器情報を知りたい。そうすることで、受け取り時に正しい機器を確認でき、安心して利用できる。

#### 受入基準

1. WHEN 予約が確定される THEN システムは予約確認メールに割り当てられた機器コード（AL1、NAL1など）を含める
2. WHEN 予約者が予約詳細画面を確認する THEN システムは割り当てられた各機器のコードとタイプを明確に表示する
3. WHEN 設置完了通知を送信する THEN システムは準備された具体的な機器コードを通知に含める
4. WHEN 予約者が返却報告をする THEN システムは返却する機器コードを確認画面に表示する
5. WHEN 複数の機器が割り当てられている THEN システムは各機器を個別に識別できるよう機器コードのリストを提供する
6. WHEN 予約者が機器受け取り時に確認する THEN システムは機器コードによる照合機能を提供する